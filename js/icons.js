/* Inline Lucide-style icons so the app works offline from file:// with no build step. */
window.LD = window.LD || {};
...
(function () {
  ...
  LD.icon = function icon(name, className) {
  ...
  LD.mountIcons = function mountIcons(root) {
    (root || document).querySelectorAll("[data-icon]").forEach((node) => {
      node.innerHTML = LD.icon(node.getAttribute("data-icon"));
    });
  };
})();
